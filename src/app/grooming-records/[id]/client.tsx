...
import { updateGroomingRecord } from "../actions";
...
export function GroomingDetailForm({
  record,
  customer,
  pet,
  pets,
  ...
  useEffect(() => {
    if (state?.success) {
      toast.success("미용 기록이 수정되었습니다.");
      router.push("/grooming-records");
    }
      ...
      <Card>
        ...
        <CardContent>
          <form action={handleSubmit} className="space-y-5">
            ...
            {/* 미용 날짜 */}
            <div className="space-y-2">
              <Label htmlFor="grooming_date">미용 날짜 *</Label>
              <Input
                id="grooming_date"
                name="grooming_date"
                type="date"
                defaultValue={record.groomingDate?.slice(0, 10)}
            ...
            {/* 설명 */}
            <div className="space-y-2">
              <Label htmlFor="description">미용 내용</Label>
              <Textarea
                id="description"
                name="description"
                defaultValue={record.description ?? ""}
                placeholder="미용 내용을 입력하세요"
                rows={4}
              />
            </div>
