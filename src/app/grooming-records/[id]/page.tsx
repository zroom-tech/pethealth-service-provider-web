import { getGroomingRecord, getUserPets } from "../actions";
import { GroomingDetailForm } from "./client";
...
export default async function GroomingDetailPage({ params }: Props) {
  ...
  const record = await getGroomingRecord(Number(id));
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold">미용 기록 상세</h1>
        <p className="text-sm text-muted-foreground">
          미용 기록을 확인하고 수정할 수 있습니다
        </p>
      </div>
      ...
      <GroomingDetailForm
        record={{
          id: record.id,
          groomingDate: record.grooming_date,
