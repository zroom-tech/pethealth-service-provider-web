...
export default async function GroomingRecordsPage({ searchParams }: Props) {
  ...
  const { records, total } = await getGroomingRecords(page);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild>
          <Link href="/grooming-records/new">
                ...
                return (
                  <TableRow key={record.id}>
                    <TableCell className="font-medium">
                      {record.grooming_date?.slice(0, 10)}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Button variant="ghost" size="icon" asChild>
                        <Link href={`/grooming-records/${record.id}`}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <Button variant="outline" size="icon" asChild disabled={page <= 1}>
            <Link href={`/grooming-records?page=${page - 1}`}>
